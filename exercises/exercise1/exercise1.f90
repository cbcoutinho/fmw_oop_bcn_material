...
program exercise1
  ...
  if(matrix_type == FULL_MATRIX) then
    ...
    deallocate ( a )
  else if(matrix_type == SYM_BAND_MATRIX) then
    call wathen_bandwidth ( nx, ny, ml, md, mu )
    ! Compute the matrix.
...
contains
  ...
  subroutine read_and_check_command_line_parameters(  NX, NY, matrix_type, solver_type )
    ...
    mcheck(len(trim(arg))>0, USAGE_ERROR_MSG)
    mcheck(trim(arg) == FULL_MATRIX .or. trim(arg) == BAND_MATRIX .or. trim(arg) == SYM_BAND_MATRIX .or. trim(arg) == SPARSE_MATRIX, MATRIX_TYPE_ERROR_MSG)
